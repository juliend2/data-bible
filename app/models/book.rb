class Book < ActiveRecord::Base
  ...
  def words_by_popularity
    ...
    excluded_words = (%w[
      ...
      que qu se ne pas du de et au tout
      un une des
      son sa ses
      mon ma mes
      le la les
      ...
      cela cet cette ces ceux celles
      plus puis encore sera où cest mais deux cette quand ans comme
      ai eut commit prit appela donna fait fit fut vit ont leva faire mit dis avez dirent avoir été jai
      le me on te car si ta là alors aussi as jusqu parce aujourd hui
    ] + (1..9).to_a + ('a'..'z').to_a + [""]).flatten.map(&:to_s).uniq
    version = Version.find_by_slug('SG21')
    vv = VerseVersion.find_by_sql <<-EOSQL
